...
const RULES: Array<{ test: RegExp; icon: LucideIcon }> = [
  // Meals of the day
  { test: /\b(breakfast|brunch|morning|colazione|desayuno|petit dejeuner|fruhstuck)\b/, icon: Sunrise },
  { test: /\b(kids?|children|ninos|bambini|enfants|kinder)\b/, icon: Baby },

  // Specific drinks
  { test: /\b(coffee|espresso|latte|cappuccino|americano|macchiato|cafe|cafeteria|kaffee|koffie)\b/, icon: Coffee },
  { test: /\b(tea|matcha|chai|the)\b/, icon: Coffee },
  { test: /\b(wine|vino|vin|wein|rose|rosado|champagne|champan|prosecco|sparkling|cava)\b/, icon: Wine },
  { test: /\b(beer|cerveza|birra|biere|bier|lager|ale|ipa|stout|pilsner|cider|sidra)\b/, icon: Beer },
  { test: /\b(cocktail|mixed drink|martini|gin|whisk|whiskey|bourbon|vodka|tequila|mezcal|rum|ron|spirit)\b/, icon: Martini },
  ...
  { test: /\b(drink|beverage|bebida|bebidas|boisson|boissons|getrank|getraenke|apero|aperitivo|aperitif)\b/, icon: GlassWater },
  ...
  { test: /\b(dessert|desserts|postre|postres|dolci|dolce|nachspeise|nachtisch|sweet|sweets|tart|brownie|cake|torta|tiramisu|pudding|flan)\b/, icon: Cake },
  ...
  { test: /\b(sandwich|panini|wrap|sub|hoagie|burger|hamburger|bap|bocadillo|torta)\b/, icon: Sandwich },
  ...
  { test: /\b(seafood|fish|pescado|pesce|poisson|shellfish|oyster|ostras|shrimp|gambas|lobster|langosta|crab|cangrejo|squid|calamar|mussel|clam)\b/, icon: Fish },
  { test: /\b(chicken|pollo|poulet|huhn|hahnchen|wing|drumstick|fried chicken)\b/, icon: Drumstick },
  ...
  { test: /\b(steak|grill|bbq|barbec|beef|lamb|cordero|pork|cerdo|ribs|costilla|chop|carne|carnes|meat|viande|fleisch)\b/, icon: Beef },

  // Mexican
  { test: /\b(taco|tacos|burrito|quesadilla|enchilada|tostada|nachos|tamale|tamales|fajita|guacamole|salsa|chilaquiles|pozole|mole)\b/, icon: Sandwich },

  // Asian (non-sushi)
  { test: /\b(noodle|noodles|udon|soba|yakisoba|pad thai|laksa|lo mein|chow mein|mee|mian)\b/, icon: UtensilsCrossed },
  { test: /\b(dim sum|bao|baozi|dumpling|gyoza|jiaozi|wonton|siu mai)\b/, icon: UtensilsCrossed },
  { test: /\b(curry|biryani|tandoori|tikka|masala|naan|dosa|chana|dal|kebab|kabob|shawarma|falafel|hummus)\b/, icon: Flame },
  { test: /\b(donburi|yakitori|tempura|tofu|onigiri|teriyaki|izakaya)\b/, icon: UtensilsCrossed },

  // Pasta / rice
  { test: /\b(pasta|spaghetti|linguine|fettuccine|penne|rigatoni|ravioli|lasagna|lasagne|gnocchi|carbonara|bolognese|primi)\b/, icon: UtensilsCrossed },
  { test: /\b(risotto|rice|arroz|riso|riz|paella|bibimbap|fried rice)\b/, icon: UtensilsCrossed },

  // Callouts
  { test: /\b(special|signature|chef|recommend|feature|featured|most popular|popular|favorito|preferito)\b/, icon: Star },
  ...
  { test: /\b(starter|appetizer|appetiser|antipasti|entree|entrada|entrant|hors d oeuvre|tapas|tapa|racion|raciones|snack|nibble|vorspeise|aperitivo)\b/, icon: Utensils },
  { test: /\b(side|sides|contorn|contorno|guarnicion|beilage|accompagnement)\b/, icon: Carrot },
  { test: /\b(main|mains|secondi|plato fuerte|plato principal|plats?|hauptgericht|hoofdgerecht|segundo|principal|dinner|lunch|course|plate)\b/, icon: UtensilsCrossed },
